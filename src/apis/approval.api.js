...
import * as Extender from './extenders/approval.apiext.js'

const moduleName = 'approval'
const headerSectionName = 'header'
const headerTableName = 'core.approval' 	
...
export default class extends Api {
	...
	// contoh: header-list
	//         header-open-data
	async init(body) { return await approval_init(this, body) }

	// header
	async headerList(body) { return await approval_headerList(this, body) }
	async headerOpen(body) { return await approval_headerOpen(this, body) }
	async headerUpdate(body) { return await approval_headerUpdate(this, body)}
	async headerCreate(body) { return await approval_headerCreate(this, body)}
	async headerDelete(body) { return await approval_headerDelete(this, body) }
	
			
}	

// init module
async function approval_init(self, body) {
	const req = self.req
	...
	try {
		...
		if (typeof Extender.approval_init === 'function') {
			// export async function approval_init(self, initialData) {}
			await Extender.approval_init(self, initialData)
		}
...
// data logging
async function approval_log(self, body, startTime, tablename, id, action, data={}, remark='') {
	const { source } = body
	const req = self.req
...
async function approval_headerList(self, body) {
	const tablename = headerTableName
	const { criteria={}, limit=0, offset=0, columns=[], sort={} } = body
	const searchMap = {
		searchtext: `approval_document = \${searchtext}`,
	};
	...
	try {
		...
		for (var row of rows) {
			...
			// lookup: approvaltype_name dari field approvaltype_name pada table core.approvaltype dimana (core.approvaltype.approvaltype_id = core.approval.approvaltype_id)
			{
...
async function approval_headerOpen(self, body) {
	const tablename = headerTableName

	try {
		const { id } = body 
		const criteria = { approval_id: id }
		const searchMap = { approval_id: `approval_id = \${approval_id}`}
		const {whereClause, queryParams} = sqlUtil.createWhereClause(criteria, searchMap) 
...
async function approval_headerCreate(self, body) {
	const { source='approval', data={} } = body
	const req = self.req
	const user_id = req.session.user.userId
	...
	try {
		...
		const result = await db.tx(async tx=>{
			...
			// generate data sesuai prefix (default: APPR) reset perbulan
			const seqdata = await sequencer.monthly(args.prefix)	
			data.approval_id = seqdata.id
			...
			// record log
			approval_log(self, body, startTime, tablename, ret.approval_id, 'CREATE', logMetadata)
...
async function approval_headerUpdate(self, body) {
	const { source='approval', data={} } = body
	const req = self.req
	const user_id = req.session.user.userId
	const startTime = process.hrtime.bigint()
	...
	try {
		...
		const result = await db.tx(async tx=>{
			...
			// eksekusi update
			const cmd = sqlUtil.createUpdateCommand(tablename, data, ['approval_id'])
			const ret = await cmd.execute(data)
			...
			// record log
			approval_log(self, body, startTime, tablename, data.approval_id, 'UPDATE')
...
async function approval_headerDelete(self, body) {
	...
	try {
		...
		const deletedRow = await db.tx(async tx=>{
			...
			const dataToRemove = {approval_id: id}
			...
			// hapus data header
			const cmd = sqlUtil.createDeleteCommand(tablename, ['approval_id'])
			...
			// record log
			approval_log(self, body, startTime, tablename, id, 'DELETE', logMetadata)
