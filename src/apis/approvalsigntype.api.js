...
import * as Extender from './extenders/approvalsigntype.apiext.js'

const moduleName = 'approvalsigntype'
const headerSectionName = 'header'
const headerTableName = 'core.approvalsigntype' 	
...
export default class extends Api {
	...
	// contoh: header-list
	//         header-open-data
	async init(body) { return await approvalsigntype_init(this, body) }

	// header
	async headerList(body) { return await approvalsigntype_headerList(this, body) }
	async headerOpen(body) { return await approvalsigntype_headerOpen(this, body) }
	async headerUpdate(body) { return await approvalsigntype_headerUpdate(this, body)}
	async headerCreate(body) { return await approvalsigntype_headerCreate(this, body)}
	async headerDelete(body) { return await approvalsigntype_headerDelete(this, body) }
	
			
}	

// init module
async function approvalsigntype_init(self, body) {
	const req = self.req
	...
	try {
		...
		if (typeof Extender.approvalsigntype_init === 'function') {
			// export async function approvalsigntype_init(self, initialData) {}
			await Extender.approvalsigntype_init(self, initialData)
		}
...
// data logging
async function approvalsigntype_log(self, body, startTime, tablename, id, action, data={}, remark='') {
	const { source } = body
	const req = self.req
...
async function approvalsigntype_headerList(self, body) {
	const tablename = headerTableName
	const { criteria={}, limit=0, offset=0, columns=[], sort={} } = body
	const searchMap = {
		searchtext: `approvalsigntype_name ILIKE '%' || \${searchtext} || '%'`,
	};
...
async function approvalsigntype_headerOpen(self, body) {
	const tablename = headerTableName

	try {
		const { id } = body 
		const criteria = { approvalsigntype_id: id }
		const searchMap = { approvalsigntype_id: `approvalsigntype_id = \${approvalsigntype_id}`}
		const {whereClause, queryParams} = sqlUtil.createWhereClause(criteria, searchMap) 
...
async function approvalsigntype_headerCreate(self, body) {
	const { source='approvalsigntype', data={} } = body
	const req = self.req
	const user_id = req.session.user.userId
	...
	try {
		...
		const result = await db.tx(async tx=>{
			...
			// record log
			approvalsigntype_log(self, body, startTime, tablename, ret.approvalsigntype_id, 'CREATE', logMetadata)
...
async function approvalsigntype_headerUpdate(self, body) {
	const { source='approvalsigntype', data={} } = body
	const req = self.req
	const user_id = req.session.user.userId
	const startTime = process.hrtime.bigint()
	...
	try {
		...
		const result = await db.tx(async tx=>{
			...
			// eksekusi update
			const cmd = sqlUtil.createUpdateCommand(tablename, data, ['approvalsigntype_id'])
			const ret = await cmd.execute(data)
			...
			// record log
			approvalsigntype_log(self, body, startTime, tablename, data.approvalsigntype_id, 'UPDATE')
...
async function approvalsigntype_headerDelete(self, body) {
	...
	try {
		...
		const deletedRow = await db.tx(async tx=>{
			...
			const dataToRemove = {approvalsigntype_id: id}
			...
			// hapus data header
			const cmd = sqlUtil.createDeleteCommand(tablename, ['approvalsigntype_id'])
			...
			// record log
			approvalsigntype_log(self, body, startTime, tablename, id, 'DELETE', logMetadata)
