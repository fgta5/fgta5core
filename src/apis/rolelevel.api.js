...
import * as Extender from './extenders/rolelevel.apiext.js'

const moduleName = 'rolelevel'
const headerSectionName = 'header'
const headerTableName = 'core.rolelevel' 	
...
export default class extends Api {
	...
	// contoh: header-list
	//         header-open-data
	async init(body) { return await rolelevel_init(this, body) }

	// header
	async headerList(body) { return await rolelevel_headerList(this, body) }
	async headerOpen(body) { return await rolelevel_headerOpen(this, body) }
	async headerUpdate(body) { return await rolelevel_headerUpdate(this, body)}
	async headerCreate(body) { return await rolelevel_headerCreate(this, body)}
	async headerDelete(body) { return await rolelevel_headerDelete(this, body) }
	
			
}	

// init module
async function rolelevel_init(self, body) {
	const req = self.req
	...
	try {
		...
		if (typeof Extender.rolelevel_init === 'function') {
			// export async function rolelevel_init(self, initialData) {}
			await Extender.rolelevel_init(self, initialData)
		}
...
// data logging
async function rolelevel_log(self, body, startTime, tablename, id, action, data={}, remark='') {
	const { source } = body
	const req = self.req
...
async function rolelevel_headerList(self, body) {
	const tablename = headerTableName
	const { criteria={}, limit=0, offset=0, columns=[], sort={} } = body
	const searchMap = {
		searchtext: `rolelevel_name ILIKE '%' || \${searchtext} || '%'`,
	};
...
async function rolelevel_headerOpen(self, body) {
	const tablename = headerTableName

	try {
		const { id } = body 
		const criteria = { rolelevel_id: id }
		const searchMap = { rolelevel_id: `rolelevel_id = \${rolelevel_id}`}
		const {whereClause, queryParams} = sqlUtil.createWhereClause(criteria, searchMap) 
...
async function rolelevel_headerCreate(self, body) {
	const { source='rolelevel', data={} } = body
	const req = self.req
	const user_id = req.session.user.userId
	...
	try {
		...
		const result = await db.tx(async tx=>{
			...
			// record log
			rolelevel_log(self, body, startTime, tablename, ret.rolelevel_id, 'CREATE', logMetadata)
...
async function rolelevel_headerUpdate(self, body) {
	const { source='rolelevel', data={} } = body
	const req = self.req
	const user_id = req.session.user.userId
	const startTime = process.hrtime.bigint()
	...
	try {
		...
		const result = await db.tx(async tx=>{
			...
			// eksekusi update
			const cmd = sqlUtil.createUpdateCommand(tablename, data, ['rolelevel_id'])
			const ret = await cmd.execute(data)
			...
			// record log
			rolelevel_log(self, body, startTime, tablename, data.rolelevel_id, 'UPDATE')
...
async function rolelevel_headerDelete(self, body) {
	...
	try {
		...
		const deletedRow = await db.tx(async tx=>{
			...
			const dataToRemove = {rolelevel_id: id}
			...
			// hapus data header
			const cmd = sqlUtil.createDeleteCommand(tablename, ['rolelevel_id'])
			...
			// record log
			rolelevel_log(self, body, startTime, tablename, id, 'DELETE', logMetadata)
