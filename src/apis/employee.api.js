...
import * as Extender from './extenders/employee.apiext.js'

const moduleName = 'employee'
const headerSectionName = 'header'
const headerTableName = 'dev.employee' 	
...
export default class extends Api {
	...
	// contoh: header-list
	//         header-open-data
	async init(body) { return await employee_init(this, body) }

	// header
	async headerList(body) { return await employee_headerList(this, body) }
	async headerOpen(body) { return await employee_headerOpen(this, body) }
	async headerUpdate(body) { return await employee_headerUpdate(this, body)}
	async headerCreate(body) { return await employee_headerCreate(this, body)}
	async headerDelete(body) { return await employee_headerDelete(this, body) }
	
			
}	

// init module
async function employee_init(self, body) {
	const req = self.req
...
// data logging
async function employee_log(self, body, startTime, tablename, id, action, data={}, remark='') {
	const { source } = body
	const req = self.req
...
async function employee_headerList(self, body) {
	const tablename = headerTableName
	const { criteria={}, limit=0, offset=0, columns=[], sort={} } = body
	const searchMap = {
		searchtext: `employe_name ILIKE '%' || \${searchtext} || '%'`,
	};
...
async function employee_headerOpen(self, body) {
	const tablename = headerTableName

	try {
		const { id } = body 
		const criteria = { employee_id: id }
		const searchMap = { employee_id: `employee_id = \${employee_id}`}
		const {whereClause, queryParams} = sqlUtil.createWhereClause(criteria, searchMap) 
...
async function employee_headerCreate(self, body) {
	const { source='employee', data={} } = body
	const req = self.req
	const user_id = req.session.user.userId
	...
	try {
		...
		const result = await db.tx(async tx=>{
			...
			// record log
			employee_log(self, body, startTime, tablename, ret.employee_id, 'CREATE', logMetadata)
...
async function employee_headerUpdate(self, body) {
	const { source='employee', data={} } = body
	const req = self.req
	const user_id = req.session.user.userId
	const startTime = process.hrtime.bigint()
	...
	try {
		...
		const result = await db.tx(async tx=>{
			...
			// eksekusi update
			const cmd = sqlUtil.createUpdateCommand(tablename, data, ['employee_id'])
			const ret = await cmd.execute(data)
			...
			// record log
			employee_log(self, body, startTime, tablename, data.employee_id, 'UPDATE')
...
async function employee_headerDelete(self, body) {
	...
	try {
		...
		const deletedRow = await db.tx(async tx=>{
			...
			const dataToRemove = {employee_id: id}
			...
			// hapus data header
			const cmd = sqlUtil.createDeleteCommand(tablename, ['employee_id'])
			...
			// record log
			employee_log(self, body, startTime, tablename, id, 'DELETE', logMetadata)
