...
import * as Extender from './extenders/gender.apiext.js'

const moduleName = 'gender'
const headerSectionName = 'header'
const headerTableName = 'core.gender' 	
...
export default class extends Api {
	...
	// contoh: header-list
	//         header-open-data
	async init(body) { return await gender_init(this, body) }

	// header
	async headerList(body) { return await gender_headerList(this, body) }
	async headerOpen(body) { return await gender_headerOpen(this, body) }
	async headerUpdate(body) { return await gender_headerUpdate(this, body)}
	async headerCreate(body) { return await gender_headerCreate(this, body)}
	async headerDelete(body) { return await gender_headerDelete(this, body) }
	
			
}	

// init module
async function gender_init(self, body) {
	const req = self.req
	...
	try {
		...
		if (typeof Extender.gender_init === 'function') {
			// export async function gender_init(self, initialData) {}
			await Extender.gender_init(self, initialData)
		}
...
// data logging
async function gender_log(self, body, startTime, tablename, id, action, data={}, remark='') {
	const { source } = body
	const req = self.req
...
async function gender_headerList(self, body) {
	const tablename = headerTableName
	const { criteria={}, limit=0, offset=0, columns=[], sort={} } = body
	const searchMap = {
		searchtext: `gender_name ILIKE '%' || \${searchtext} || '%'`,
	};
...
async function gender_headerOpen(self, body) {
	const tablename = headerTableName

	try {
		const { id } = body 
		const criteria = { gender_id: id }
		const searchMap = { gender_id: `gender_id = \${gender_id}`}
		const {whereClause, queryParams} = sqlUtil.createWhereClause(criteria, searchMap) 
...
async function gender_headerCreate(self, body) {
	const { source='gender', data={} } = body
	const req = self.req
	const user_id = req.session.user.userId
	...
	try {
		...
		const result = await db.tx(async tx=>{
			...
			// record log
			gender_log(self, body, startTime, tablename, ret.gender_id, 'CREATE', logMetadata)
...
async function gender_headerUpdate(self, body) {
	const { source='gender', data={} } = body
	const req = self.req
	const user_id = req.session.user.userId
	const startTime = process.hrtime.bigint()
	...
	try {
		...
		const result = await db.tx(async tx=>{
			...
			// eksekusi update
			const cmd = sqlUtil.createUpdateCommand(tablename, data, ['gender_id'])
			const ret = await cmd.execute(data)
			...
			// record log
			gender_log(self, body, startTime, tablename, data.gender_id, 'UPDATE')
...
async function gender_headerDelete(self, body) {
	...
	try {
		...
		const deletedRow = await db.tx(async tx=>{
			...
			const dataToRemove = {gender_id: id}
			...
			// hapus data header
			const cmd = sqlUtil.createDeleteCommand(tablename, ['gender_id'])
			...
			// record log
			gender_log(self, body, startTime, tablename, id, 'DELETE', logMetadata)
