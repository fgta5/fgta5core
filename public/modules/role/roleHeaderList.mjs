...
const CurrentSectionId = Context.Sections.roleHeaderList
const CurrentSection = Crsl.Items[CurrentSectionId]
const CurrentState = {}

const tbl =  new $fgta5.Gridview('roleHeaderList-tbl')
const pnl_search = document.getElementById('roleHeaderList-pnl_search')
const btn_gridload = new $fgta5.ActionButton('roleHeaderList-btn_gridload') 

export const Section = CurrentSection
export const SearchParams = {}

export async function init(self, args) {
	console.log('initializing roleHeaderList ...')
	...
	try {
		...
		const obj_searchtext = document.getElementById('roleHeaderList-txt_searchtext')
		const btn_load = document.getElementById('roleHeaderList-btn_gridload')
		...
		// roleHeaderList-ext.mjs, export function initSearchParams(self, SearchParams) {} 
...
async function openRow(self, tr) {
	...
	const roleHeaderEdit = self.Modules.roleHeaderEdit
	roleHeaderEdit.clearForm(self, 'loading...')

	try {
		setCurrentRow(self, tr)
		CurrentState.SelectedRow.keyValue = keyvalue
		CurrentState.SelectedRow.key = key
		await roleHeaderEdit.openSelectedData(self, {key:key, keyvalue:keyvalue})

		// matikan atau nyalakan button prev/next sesuai kondisi
		setPagingButton(self, roleHeaderEdit)
...
function tbl_cellclick(self, evt) {
	...
	const roleHeaderEdit = self.Modules.roleHeaderEdit
	roleHeaderEdit.Section.show({}, (evt)=>{
		openRow(self, tr)
	})
...
async function tbl_loadData(self, params={}) {
	console.log('loading roleHeader data')
	console.log(params)
