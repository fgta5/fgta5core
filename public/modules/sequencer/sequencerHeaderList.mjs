...
const CurrentSectionId = Context.Sections.sequencerHeaderList
const CurrentSection = Crsl.Items[CurrentSectionId]
const CurrentState = {}

const tbl =  new $fgta5.Gridview('sequencerHeaderList-tbl')
const pnl_search = document.getElementById('sequencerHeaderList-pnl_search')
const btn_gridload = new $fgta5.ActionButton('sequencerHeaderList-btn_gridload') 

export const Section = CurrentSection
export const SearchParams = {}

export async function init(self, args) {
	console.log('initializing sequencerHeaderList ...')
	...
	try {
		...
		// sequencerHeaderList-ext.mjs, export function initSearchParams(self, SearchParams) {} 
...
export async function render(self) {
	console.log('sequencerHeaderList render')
}
...
async function openRow(self, tr) {
	...
	const sequencerHeaderEdit = self.Modules.sequencerHeaderEdit

	try {
		setCurrentRow(self, tr)
		CurrentState.SelectedRow.keyValue = keyvalue
		CurrentState.SelectedRow.key = key
		await sequencerHeaderEdit.openSelectedData(self, {key:key, keyvalue:keyvalue})
	} catch (err) {
		console.error(err)
...
async function tbl_celldblclick(self, evt) {
	...
	const sequencerHeaderEdit = self.Modules.sequencerHeaderEdit
	sequencerHeaderEdit.Section.show()
...
async function tbl_loadData(self, params={}) {
	console.log('loading sequencerHeader data')
	console.log(params)
