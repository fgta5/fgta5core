import Context from './sequencer-context.mjs'
import * as Extender from './sequencer-ext.mjs'


const CurrentState = {}
const Crsl =  Context.Crsl
const CurrentSectionId = Context.Sections.sequencerHeaderEdit
const CurrentSection = Crsl.Items[CurrentSectionId]
const Source = Context.Source


const TitleWhenNew = 'New Sequencer'
const TitleWhenView = 'View Sequencer'
const TitleWhenEdit = 'Edit Sequencer'
const EditModeText = 'Edit'
const LockModeText = 'Lock'

const btn_edit = new $fgta5.ActionButton('sequencerHeaderEdit-btn_edit')
const btn_save = new $fgta5.ActionButton('sequencerHeaderEdit-btn_save')
const btn_new = new $fgta5.ActionButton('sequencerHeaderEdit-btn_new', 'sequencerHeader-new')
const btn_del = new $fgta5.ActionButton('sequencerHeaderEdit-btn_delete')
const btn_reset = new $fgta5.ActionButton('sequencerHeaderEdit-btn_reset')
const btn_prev = new $fgta5.ActionButton('sequencerHeaderEdit-btn_prev')
const btn_next = new $fgta5.ActionButton('sequencerHeaderEdit-btn_next')

const frm = new $fgta5.Form('sequencerHeaderEdit-frm');
const obj_sequencer_id = frm.Inputs['sequencerHeaderEdit-obj_sequencer_id']
const obj_sequencer_year = frm.Inputs['sequencerHeaderEdit-obj_sequencer_year']
const obj_sequencer_month = frm.Inputs['sequencerHeaderEdit-obj_sequencer_month']
const obj_sequencer_seqnum = frm.Inputs['sequencerHeaderEdit-obj_sequencer_seqnum']
const obj_sequencer_block = frm.Inputs['sequencerHeaderEdit-obj_sequencer_block']
const obj_sequencer_cluster = frm.Inputs['sequencerHeaderEdit-obj_sequencer_cluster']
const obj_sequencer_number = frm.Inputs['sequencerHeaderEdit-obj_sequencer_number']
const obj_sequencer_lastdate = frm.Inputs['sequencerHeaderEdit-obj_sequencer_lastdate']
const obj_sequencer_remark = frm.Inputs['sequencerHeaderEdit-obj_sequencer_remark']	


export const Section = CurrentSection

export async function init(self, args) {
	console.log('initializing sequencerHeaderEdit ...')
...
export async function render(self) {
	console.log('sequencerHeaderEdit render')
}
...
async function btn_new_click(self, evt) {
	...
	const sequencerHeaderList = self.Modules.sequencerHeaderList
	const listsecid = sequencerHeaderList.Section.Id
	...
	try {
		...
		if (fromListSection) {
			...
			self.Modules.sequencerHeaderList.Section.show()
		}
	}
}
...
async function btn_save_click(self, evt) {
	...
	try {
		...
		if (isNewData) {
			...
			self.Modules.sequencerHeaderList.addNewRow(self, data)
		} else {
			console.log('update data baris yang dibuka')
			self.Modules.sequencerHeaderList.updateCurrentRow(self, data)
		}
...
async function btn_del_click(self, evt) {
	...
	try {
		...
		self.Modules.sequencerHeaderList.removeCurrentRow(self)
...
async function btn_prev_click(self, evt) {
	...
	self.Modules.sequencerHeaderList.selectPreviousRow(self)
}

async function btn_next_click(self, evt) {
	console.log('btn_next_click')
	self.Modules.sequencerHeaderList.selectNextRow(self)
}
