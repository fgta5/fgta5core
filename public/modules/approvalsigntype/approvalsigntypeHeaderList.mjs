...
const CurrentSectionId = Context.Sections.approvalsigntypeHeaderList
const CurrentSection = Crsl.Items[CurrentSectionId]
const CurrentState = {}

const tbl =  new $fgta5.Gridview('approvalsigntypeHeaderList-tbl')
const pnl_search = document.getElementById('approvalsigntypeHeaderList-pnl_search')
const btn_gridload = new $fgta5.ActionButton('approvalsigntypeHeaderList-btn_gridload') 

export const Section = CurrentSection
export const SearchParams = {}

export async function init(self, args) {
	console.log('initializing approvalsigntypeHeaderList ...')
	...
	try {
		...
		const obj_searchtext = document.getElementById('approvalsigntypeHeaderList-txt_searchtext')
		const btn_load = document.getElementById('approvalsigntypeHeaderList-btn_gridload')
		...
		// approvalsigntypeHeaderList-ext.mjs, export function initSearchParams(self, SearchParams) {} 
...
export function keyboardAction(self, actionName, evt) {
	if (actionName=='up') {
		...
		const approvalsigntypeHeaderEdit = self.Modules.approvalsigntypeHeaderEdit
		if (tbl.CurrentRow!=null) {
			approvalsigntypeHeaderEdit.Section.show({}, ()=>{
...
async function openRow(self, tr) {
	...
	const approvalsigntypeHeaderEdit = self.Modules.approvalsigntypeHeaderEdit
	approvalsigntypeHeaderEdit.clearForm(self, 'loading...')

	try {
		setCurrentRow(self, tr)
		CurrentState.SelectedRow.keyValue = keyvalue
		CurrentState.SelectedRow.key = key
		await approvalsigntypeHeaderEdit.openSelectedData(self, {key:key, keyvalue:keyvalue})

		// matikan atau nyalakan button prev/next sesuai kondisi
		setPagingButton(self, approvalsigntypeHeaderEdit)
...
function tbl_cellclick(self, evt) {
	...
	const approvalsigntypeHeaderEdit = self.Modules.approvalsigntypeHeaderEdit
	approvalsigntypeHeaderEdit.Section.show({}, (evt)=>{
		openRow(self, tr)
	})
...
async function tbl_loadData(self, params={}) {
	console.log('loading approvalsigntypeHeader data')
	console.log(params)
