import Context from './approvalsigntype-context.mjs'
import * as Ext from './approvalsigntype-ext.mjs'
...
const CurrentSectionId = Context.Sections.approvalsigntypeHeaderEdit
const CurrentSection = Crsl.Items[CurrentSectionId]
const Source = Context.Source


const TitleWhenNew = 'New Sign Type'
const TitleWhenView = 'View Sign Type'
const TitleWhenEdit = 'Edit Sign Type'
const EditModeText = 'Edit'
const LockModeText = 'Lock'

const btn_edit = new $fgta5.ActionButton('approvalsigntypeHeaderEdit-btn_edit')
const btn_save = new $fgta5.ActionButton('approvalsigntypeHeaderEdit-btn_save')
const btn_new = new $fgta5.ActionButton('approvalsigntypeHeaderEdit-btn_new', 'approvalsigntypeHeader-new')
const btn_del = new $fgta5.ActionButton('approvalsigntypeHeaderEdit-btn_delete')
const btn_reset = new $fgta5.ActionButton('approvalsigntypeHeaderEdit-btn_reset')
const btn_prev = new $fgta5.ActionButton('approvalsigntypeHeaderEdit-btn_prev')
const btn_next = new $fgta5.ActionButton('approvalsigntypeHeaderEdit-btn_next')


const btn_recordstatus = document.getElementById('approvalsigntypeHeader-btn_recordstatus')
const btn_logs = document.getElementById('approvalsigntypeHeader-btn_logs')
const btn_about = document.getElementById('approvalsigntypeHeader-btn_about')

const frm = new $fgta5.Form('approvalsigntypeHeaderEdit-frm');
const obj_approvalsigntype_id = frm.Inputs['approvalsigntypeHeaderEdit-obj_approvalsigntype_id']
const obj_approvalsigntype_name = frm.Inputs['approvalsigntypeHeaderEdit-obj_approvalsigntype_name']	
...
export async function init(self, args) {
	console.log('initializing approvalsigntypeHeaderEdit ...')
...
export async function openSelectedData(self, params) {
	...
	try {
		...
		const fn_iseditdisabled_name = 'approvalsigntypeHeaderEdit_isEditDisabled'
		...
		// export async function approvalsigntypeHeaderEdit_formOpened(self, frm, CurrentState)
		const fn_formopened_name = 'approvalsigntypeHeaderEdit_formOpened'
...
async function backToList(self, evt) {
	...
	if (goback) {
		frm.lock()
		const listId =  Context.Sections.approvalsigntypeHeaderList
...
async function  frm_locked(self, evt) {
	...
	// export function approvalsigntypeHeaderEdit_formLocked(self, frm, CurrentState) {}
	const fn_name = 'approvalsigntypeHeaderEdit_formLocked'
...
async function  frm_unlocked(self, evt) {
	...
	// export function approvalsigntypeHeaderEdit_formUnlocked(self, frm, CurrentState) {}
	const fn_name = 'approvalsigntypeHeaderEdit_formUnlocked'
	const fn = Extender[fn_name]
	if (typeof fn === 'function') {
		fn(self, frm, CurrentState)
	}
...
async function btn_new_click(self, evt) {
	...
	const approvalsigntypeHeaderList = self.Modules.approvalsigntypeHeaderList
	const listsecid = approvalsigntypeHeaderList.Section.Id
	...
	try {
		...
		const fn_newdata_name = 'approvalsigntypeHeaderEdit_newData'
		const fn_newdata = Extender[fn_newdata_name]
		if (typeof fn_newdata === 'function') {
			// export async function approvalsigntypeHeaderEdit_newData(self, datainit, frm) {}
		...
		if (fromListSection) {
			...
			self.Modules.approvalsigntypeHeaderList.Section.show()
		}
	}
}
...
async function btn_save_click(self, evt) {
	...
	// Extender Autofill
	const fn_autofill_name = 'approvalsigntypeHeaderEdit_autofill'
	...
	// Extender Saving
	// export async function approvalsigntypeHeaderEdit_dataSaving(self, dataToSave, frm, args) {}
	const args = { cancelSave: false }
	const fn_datasaving_name = 'approvalsigntypeHeaderEdit_dataSaving'
	...
	try {
		...
		// Extender Saving
		const fn_datasaved_name = 'approvalsigntypeHeaderEdit_dataSaved'
		const fn_datasaved = Extender[fn_datasaved_name]
		if (typeof fn_datasaved === 'function') {
			// export async function approvalsigntypeHeaderEdit_dataSaved(self, data, frm) {}
		...
		if (isNewData) {
			...
			self.Modules.approvalsigntypeHeaderList.addNewRow(self, data)
		} else {
			console.log('update data baris yang dibuka')
			self.Modules.approvalsigntypeHeaderList.updateCurrentRow(self, data)
		}
...
async function btn_del_click(self, evt) {
	...
	try {
		...
		self.Modules.approvalsigntypeHeaderList.removeCurrentRow(self)
...
async function btn_prev_click(self, evt) {
	...
	self.Modules.approvalsigntypeHeaderList.selectPreviousRow(self)
}

async function btn_next_click(self, evt) {
	console.log('btn_next_click')
	self.Modules.approvalsigntypeHeaderList.selectNextRow(self)
}
...
async function btn_recordstatus_click(self, evt) {
	...
	pageHelper.openSection(self, 'fRecord-section', params, async ()=>{
		...
		try {
			...
			const fn_addrecordinfo_name = 'approvalsigntypeHeaderEdit_addRecordInfo'
...
async function btn_about_click(self, evt) {
	...
	pageHelper.openSection(self, 'fAbout-section', params, async ()=>{
		...
		AboutSection.Title = 'About Sign Type'
		...
		if ( document.getElementById('fAbout-section-footer') == null) {
			...
			divFooter.innerHTML = 'This module is generated by fgta5 generator at 9 Dec 2025 09:56'
