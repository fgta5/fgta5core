...
const CurrentSectionId = Context.Sections.docHeaderList
const CurrentSection = Crsl.Items[CurrentSectionId]
const CurrentState = {}

const tbl =  new $fgta5.Gridview('docHeaderList-tbl')
const pnl_search = document.getElementById('docHeaderList-pnl_search')
const btn_gridload = new $fgta5.ActionButton('docHeaderList-btn_gridload') 

export const Section = CurrentSection
export const SearchParams = {}

export async function init(self, args) {
	console.log('initializing docHeaderList ...')
	...
	try {
		...
		const obj_searchtext = document.getElementById('docHeaderList-txt_searchtext')
		const btn_load = document.getElementById('docHeaderList-btn_gridload')
		...
		// docHeaderList-ext.mjs, export function initSearchParams(self, SearchParams) {} 
...
export function keyboardAction(self, actionName, evt) {
	if (actionName=='up') {
		...
		const docHeaderEdit = self.Modules.docHeaderEdit
		if (tbl.CurrentRow!=null) {
			docHeaderEdit.Section.show({}, ()=>{
...
async function openRow(self, tr) {
	...
	const docHeaderEdit = self.Modules.docHeaderEdit
	docHeaderEdit.clearForm(self, 'loading...')

	try {
		setCurrentRow(self, tr)
		CurrentState.SelectedRow.keyValue = keyvalue
		CurrentState.SelectedRow.key = key
		await docHeaderEdit.openSelectedData(self, {key:key, keyvalue:keyvalue})
	} catch (err) {
		console.error(err)
	...
	setPagingButton(self, docHeaderEdit)
}
...
function tbl_cellclick(self, evt) {
	...
	const docHeaderEdit = self.Modules.docHeaderEdit
	docHeaderEdit.Section.show({}, (evt)=>{
		openRow(self, tr)
	})
...
async function tbl_loadData(self, params={}) {
	console.log('loading docHeader data')
	console.log(params)
