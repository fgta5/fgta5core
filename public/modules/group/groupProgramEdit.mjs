...
const CurrentSectionId = Context.Sections.groupProgramEdit
const CurrentSection = Crsl.Items[CurrentSectionId]
const Source = Context.Source

const TitleWhenNew = 'New Programs'
const TitleWhenView = 'View Programs'
const TitleWhenEdit = 'Edit Programs'
const EditModeText = 'Edit'
const LockModeText = 'Lock'



const btn_edit = new $fgta5.ActionButton('groupProgramEdit-btn_edit')
const btn_save = new $fgta5.ActionButton('groupProgramEdit-btn_save')
const btn_new = new $fgta5.ActionButton('groupProgramEdit-btn_new', 'groupProgram-addrow')
const btn_del = new $fgta5.ActionButton('groupProgramEdit-btn_delete', 'groupProgram-delrow')
const btn_reset = new $fgta5.ActionButton('groupProgramEdit-btn_reset')
const btn_prev = new $fgta5.ActionButton('groupProgramEdit-btn_prev')
const btn_next = new $fgta5.ActionButton('groupProgramEdit-btn_next')

const btn_recordstatus = document.getElementById('groupProgram-btn_recordstatus')
const btn_logs = document.getElementById('groupProgram-btn_logs')
...
export async function init(self, args) {
	...
	CurrentState.getHeaderForm = () => {
		...
		const frmHeader = groupHeaderEdit.getHeaderForm()
		return frmHeader
	}
	...
	obj_program_id.addEventListener('selecting', async (evt)=>{
		...
		if (typeof fn_selecting === 'function') {
			...
			fn_selecting(self, obj_program_id, frm, evt)
		} else {
			// default selecting
			...
			// export function obj_program_id_selecting_criteria(self, obj_program_id, frm, criteria, sort, evt) {}
			...
			if (typeof fn_selecting_criteria === 'function') {
				fn_selecting_criteria(self, obj_program_id, frm, criteria, sort, evt)
			}
...
export async function openSelectedData(self, params) {
	...
	try {
		...
		if (!CurrentState.headerFormLocked) {
			const fn_iseditdisabled_name = 'groupProgramEdit_isEditDisabled'
		...
		// export function groupProgramEdit_formOpened(self, frm, CurrentState) {}
		const fn_formopened_name = 'groupProgramEdit_formOpened'
...
export function headerLocked(self) {
	...
	// export function groupProgramEdit_formLocked(self, frm, CurrentState) {}
...
export function headerUnlocked(self) {
	...
	// export function groupProgramEdit_formUnlocked(self, frm, CurrentState) {}
...
async function openData(self, id) {
	const url = `/${Context.moduleName}/program-open`
	try {
...
async function createData(self, data, formData) {
	const url = `/${Context.moduleName}/program-create`
	try {
...
async function updateData(self, data, formData) {
	const url = `/${Context.moduleName}/program-update`
	try {
		const result = await Module.apiCall(url, { data, source: Source }, formData) 
		return result 
	} catch (err) {
...
async function deleteData(self, id) {
	const url = `/${Context.moduleName}/program-delete`
	try {
...
async function backToList(self, evt) {
	...
	if (goback) {
		frm.lock()
		const listId =  Context.Sections.groupProgramList
...
async function  frm_unlocked(self, evt) {
	...
	// Extender untuk event Unlocked
	// export function groupProgramEdit_formUnlocked(self, frm) {}
	const fn_name = 'groupProgramEdit_formUnlocked'
	const fn = Extender[fn_name]
	if (typeof fn === 'function') {
		fn(self, frm)
...
async function btn_new_click(self, evt) {
	...
	const groupProgramList = self.Modules.groupProgramList
	const listsecid = groupProgramList.Section.Id
	const fromListSection = sourceSection===listsecid

	if (fromListSection) {
		console.log('tambahkan row baru')
		CurrentSection.setSectionReturn(groupProgramList.Section)
	...
	try {
		...
		const group_id = header_pk.value
		...
		// export async function groupProgramEdit_newData(self, datainit, frm, CurrentState) {}
		const fn_newdata_name = 'groupProgramEdit_newData'
		...
		if (fromListSection) {
			...
			self.Modules.groupProgramList.Section.show()
		}
	}
}
...
async function btn_save_click(self, evt) {
	...
	// Extender Autofill
	// export async function groupProgramEdit_autofill(self, frm) {}
	...
	// Extender Saving
	// export async function groupProgramEdit_dataSaving(self, dataToSave, frm, args) {}
	const args = { cancelSave: false }
	const fn_datasaving_name = 'groupProgramEdit_dataSaving'
	...
	try {
		...
		// Extender Saving
		// export async function groupProgramEdit_dataSaved(self, data, frm) {}
		const fn_datasaved_name = 'groupProgramEdit_dataSaved'
		...
		if (isNewData) {
			...
			self.Modules.groupProgramList.addNewRow(self, data)
		} else {
			console.log('update data baris yang dibuka')
			self.Modules.groupProgramList.updateCurrentRow(self, data)
		}
...
async function btn_del_click(self, evt) {
	...
	try {
		...
		self.Modules.groupProgramList.removeCurrentRow(self)
...
async function btn_prev_click(self, evt) {
	...
	self.Modules.groupProgramList.selectPreviousRow(self)
}

async function btn_next_click(self, evt) {
	console.log('btn_next_click')
	self.Modules.groupProgramList.selectNextRow(self)
}
...
async function btn_recordstatus_click(self, evt) {
	...
	pageHelper.openSection(self, 'fRecord-section', params, async ()=>{
		...
		try {
			...
			// melalui extender groupProgramEdit_addRecordInfo
			// export async function groupProgramEdit_addRecordInfo(self,  data) {}
