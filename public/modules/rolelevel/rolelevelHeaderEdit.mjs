import Context from './rolelevel-context.mjs'
import * as Ext from './rolelevel-ext.mjs'
...
const CurrentSectionId = Context.Sections.rolelevelHeaderEdit
const CurrentSection = Crsl.Items[CurrentSectionId]
const Source = Context.Source


const TitleWhenNew = 'New Role Level'
const TitleWhenView = 'View Role Level'
const TitleWhenEdit = 'Edit Role Level'
const EditModeText = 'Edit'
const LockModeText = 'Lock'

const btn_edit = new $fgta5.ActionButton('rolelevelHeaderEdit-btn_edit')
const btn_save = new $fgta5.ActionButton('rolelevelHeaderEdit-btn_save')
const btn_new = new $fgta5.ActionButton('rolelevelHeaderEdit-btn_new', 'rolelevelHeader-new')
const btn_del = new $fgta5.ActionButton('rolelevelHeaderEdit-btn_delete')
const btn_reset = new $fgta5.ActionButton('rolelevelHeaderEdit-btn_reset')
const btn_prev = new $fgta5.ActionButton('rolelevelHeaderEdit-btn_prev')
const btn_next = new $fgta5.ActionButton('rolelevelHeaderEdit-btn_next')


const btn_recordstatus = document.getElementById('rolelevelHeader-btn_recordstatus')
const btn_logs = document.getElementById('rolelevelHeader-btn_logs')
const btn_about = document.getElementById('rolelevelHeader-btn_about')

const frm = new $fgta5.Form('rolelevelHeaderEdit-frm');
const obj_rolelevel_id = frm.Inputs['rolelevelHeaderEdit-obj_rolelevel_id']
const obj_rolelevel_name = frm.Inputs['rolelevelHeaderEdit-obj_rolelevel_name']
const obj_rolelevel_descr = frm.Inputs['rolelevelHeaderEdit-obj_rolelevel_descr']
const obj_rolelevel_order = frm.Inputs['rolelevelHeaderEdit-obj_rolelevel_order']	
...
export async function init(self, args) {
	console.log('initializing rolelevelHeaderEdit ...')
	...
	// export async function rolelevelHeaderEdit_init(self, CurrentState)
	const fn_init_name = 'rolelevelHeaderEdit_init'
...
export async function openSelectedData(self, params) {
	...
	try {
		...
		// export async function rolelevelHeaderEdit_isEditDisabled(self, data)
		const fn_iseditdisabled_name = 'rolelevelHeaderEdit_isEditDisabled'
		...
		// export async function rolelevelHeaderEdit_formOpened(self, frm, CurrentState)
		const fn_formopened_name = 'rolelevelHeaderEdit_formOpened'
...
async function backToList(self, evt) {
	...
	if (goback) {
		frm.lock()
		const listId =  Context.Sections.rolelevelHeaderList
...
async function  frm_locked(self, evt) {
	...
	// export function rolelevelHeaderEdit_formLocked(self, frm, CurrentState) {}
	const fn_name = 'rolelevelHeaderEdit_formLocked'
...
async function  frm_unlocked(self, evt) {
	...
	// export function rolelevelHeaderEdit_formUnlocked(self, frm, CurrentState) {}
	const fn_name = 'rolelevelHeaderEdit_formUnlocked'
	const fn = Extender[fn_name]
	if (typeof fn === 'function') {
		fn(self, frm, CurrentState)
	}
...
async function btn_new_click(self, evt) {
	...
	const rolelevelHeaderList = self.Modules.rolelevelHeaderList
	const listsecid = rolelevelHeaderList.Section.Id
	...
	try {
		...
		const fn_newdata_name = 'rolelevelHeaderEdit_newData'
		const fn_newdata = Extender[fn_newdata_name]
		if (typeof fn_newdata === 'function') {
			// export async function rolelevelHeaderEdit_newData(self, datainit, frm) {}
		...
		if (fromListSection) {
			...
			self.Modules.rolelevelHeaderList.Section.show()
		}
	}
}
...
async function btn_save_click(self, evt) {
	...
	// Extender Autofill
	const fn_autofill_name = 'rolelevelHeaderEdit_autofill'
	...
	// Extender Saving
	// export async function rolelevelHeaderEdit_dataSaving(self, dataToSave, frm, args) {}
	const args = { cancelSave: false }
	const fn_datasaving_name = 'rolelevelHeaderEdit_dataSaving'
	...
	try {
		...
		// Extender Saving
		const fn_datasaved_name = 'rolelevelHeaderEdit_dataSaved'
		const fn_datasaved = Extender[fn_datasaved_name]
		if (typeof fn_datasaved === 'function') {
			// export async function rolelevelHeaderEdit_dataSaved(self, data, frm) {}
		...
		if (isNewData) {
			...
			self.Modules.rolelevelHeaderList.addNewRow(self, data)
		} else {
			console.log('update data baris yang dibuka')
			self.Modules.rolelevelHeaderList.updateCurrentRow(self, data)
		}
...
async function btn_del_click(self, evt) {
	...
	try {
		...
		self.Modules.rolelevelHeaderList.removeCurrentRow(self)
...
async function btn_prev_click(self, evt) {
	...
	self.Modules.rolelevelHeaderList.selectPreviousRow(self)
}

async function btn_next_click(self, evt) {
	console.log('btn_next_click')
	self.Modules.rolelevelHeaderList.selectNextRow(self)
}
...
async function btn_recordstatus_click(self, evt) {
	...
	pageHelper.openSection(self, 'fRecord-section', params, async ()=>{
		...
		try {
			...
			const fn_addrecordinfo_name = 'rolelevelHeaderEdit_addRecordInfo'
...
async function btn_about_click(self, evt) {
	...
	pageHelper.openSection(self, 'fAbout-section', params, async ()=>{
		...
		AboutSection.Title = 'About Role Level'
		...
		if ( document.getElementById('fAbout-section-footer') == null) {
			...
			divFooter.innerHTML = 'This module is generated by fgta5 generator at 17 Dec 2025 09:08'
