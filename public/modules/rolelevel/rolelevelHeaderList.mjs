...
const CurrentSectionId = Context.Sections.rolelevelHeaderList
const CurrentSection = Crsl.Items[CurrentSectionId]
const CurrentState = {}

const tbl =  new $fgta5.Gridview('rolelevelHeaderList-tbl')
const pnl_search = document.getElementById('rolelevelHeaderList-pnl_search')
const btn_gridload = new $fgta5.ActionButton('rolelevelHeaderList-btn_gridload') 

export const Section = CurrentSection
export const SearchParams = {}

export async function init(self, args) {
	console.log('initializing rolelevelHeaderList ...')
	...
	try {
		...
		const obj_searchtext = document.getElementById('rolelevelHeaderList-txt_searchtext')
		const btn_load = document.getElementById('rolelevelHeaderList-btn_gridload')
		...
		// rolelevelHeaderList-ext.mjs, export function initSearchParams(self, SearchParams) {} 
...
export function keyboardAction(self, actionName, evt) {
	if (actionName=='up') {
		...
		const rolelevelHeaderEdit = self.Modules.rolelevelHeaderEdit
		if (tbl.CurrentRow!=null) {
			rolelevelHeaderEdit.Section.show({}, ()=>{
...
async function openRow(self, tr) {
	...
	const rolelevelHeaderEdit = self.Modules.rolelevelHeaderEdit
	rolelevelHeaderEdit.clearForm(self, 'loading...')

	try {
		setCurrentRow(self, tr)
		CurrentState.SelectedRow.keyValue = keyvalue
		CurrentState.SelectedRow.key = key
		await rolelevelHeaderEdit.openSelectedData(self, {key:key, keyvalue:keyvalue})

		// matikan atau nyalakan button prev/next sesuai kondisi
		setPagingButton(self, rolelevelHeaderEdit)
...
function tbl_cellclick(self, evt) {
	...
	const rolelevelHeaderEdit = self.Modules.rolelevelHeaderEdit
	rolelevelHeaderEdit.Section.show({}, (evt)=>{
		openRow(self, tr)
	})
...
async function tbl_loadData(self, params={}) {
	console.log('loading rolelevelHeader data')
	console.log(params)
