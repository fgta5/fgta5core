import Context from './interface-context.mjs'
import * as Extender from './interface-ext.mjs'
import * as pageHelper from '/public/libs/webmodule/pagehelper.mjs'

const CurrentState = {}
const Crsl =  Context.Crsl
const CurrentSectionId = Context.Sections.interfaceHeaderEdit
const CurrentSection = Crsl.Items[CurrentSectionId]
const Source = Context.Source


const TitleWhenNew = 'New Interface'
const TitleWhenView = 'View Interface'
const TitleWhenEdit = 'Edit Interface'
const EditModeText = 'Edit'
const LockModeText = 'Lock'

const btn_edit = new $fgta5.ActionButton('interfaceHeaderEdit-btn_edit')
const btn_save = new $fgta5.ActionButton('interfaceHeaderEdit-btn_save')
const btn_new = new $fgta5.ActionButton('interfaceHeaderEdit-btn_new', 'interfaceHeader-new')
const btn_del = new $fgta5.ActionButton('interfaceHeaderEdit-btn_delete')
const btn_reset = new $fgta5.ActionButton('interfaceHeaderEdit-btn_reset')
const btn_prev = new $fgta5.ActionButton('interfaceHeaderEdit-btn_prev')
const btn_next = new $fgta5.ActionButton('interfaceHeaderEdit-btn_next')


const btn_recordstatus = document.getElementById('interfaceHeader-btn_recordstatus')
const btn_logs = document.getElementById('interfaceHeader-btn_logs')
const btn_about = document.getElementById('interfaceHeader-btn_about')

const frm = new $fgta5.Form('interfaceHeaderEdit-frm');
const obj_interface_id = frm.Inputs['interfaceHeaderEdit-obj_interface_id']
const obj_interface_name = frm.Inputs['interfaceHeaderEdit-obj_interface_name']	
...
export async function init(self, args) {
	console.log('initializing interfaceHeaderEdit ...')
...
export async function openSelectedData(self, params) {
	...
	try {
		...
		const fn_iseditdisabled_name = 'interfaceHeaderEdit_isEditDisabled'
		...
		const fn_formopened_name = 'interfaceHeaderEdit_formOpened'
		const fn_formopened = Extender[fn_formopened_name]
		if (typeof fn_formopened === 'function') {
			// export async function interfaceHeaderEdit_formOpened(self, frm, CurrentState)
...
async function backToList(self, evt) {
	...
	if (goback) {
		frm.lock()
		const listId =  Context.Sections.interfaceHeaderList
...
async function  frm_locked(self, evt) {
	...
	const fn_name = 'interfaceHeaderEdit_formLocked'
...
async function  frm_unlocked(self, evt) {
	...
	const fn_name = 'interfaceHeaderEdit_formUnlocked'
	const fn = Extender[fn_name]
	if (typeof fn === 'function') {
		fn(self, frm, CurrentState)
	}
...
async function btn_new_click(self, evt) {
	...
	const interfaceHeaderList = self.Modules.interfaceHeaderList
	const listsecid = interfaceHeaderList.Section.Id
	...
	try {
		...
		const fn_newdata_name = 'interfaceHeaderEdit_newData'
		...
		if (fromListSection) {
			...
			self.Modules.interfaceHeaderList.Section.show()
		}
	}
}
...
async function btn_save_click(self, evt) {
	...
	// Extender Autofill
	const fn_autofill_name = 'interfaceHeaderEdit_autofill'
	...
	// Extender Saving
	const fn_datasaving_name = 'interfaceHeaderEdit_dataSaving'
	...
	try {
		...
		// Extender Saving
		const fn_datasaved_name = 'interfaceHeaderEdit_dataSaved'
		...
		if (isNewData) {
			...
			self.Modules.interfaceHeaderList.addNewRow(self, data)
		} else {
			console.log('update data baris yang dibuka')
			self.Modules.interfaceHeaderList.updateCurrentRow(self, data)
		}
...
async function btn_del_click(self, evt) {
	...
	try {
		...
		self.Modules.interfaceHeaderList.removeCurrentRow(self)
...
async function btn_prev_click(self, evt) {
	...
	self.Modules.interfaceHeaderList.selectPreviousRow(self)
}

async function btn_next_click(self, evt) {
	console.log('btn_next_click')
	self.Modules.interfaceHeaderList.selectNextRow(self)
}
...
async function btn_recordstatus_click(self, evt) {
	...
	pageHelper.openSection(self, 'fRecord-section', params, async ()=>{
		...
		try {
			...
			const fn_addrecordinfo_name = 'interfaceHeaderEdit_addRecordInfo'
...
async function btn_about_click(self, evt) {
	...
	pageHelper.openSection(self, 'fAbout-section', params, async ()=>{
		...
		AboutSection.Title = 'About Interface'
		...
		if ( document.getElementById('fAbout-section-footer') == null) {
			...
			divFooter.innerHTML = 'This module is generated by fgta5 generator at 5 Nov 2025 15:12'
