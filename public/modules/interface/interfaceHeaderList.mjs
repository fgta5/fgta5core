...
const CurrentSectionId = Context.Sections.interfaceHeaderList
const CurrentSection = Crsl.Items[CurrentSectionId]
const CurrentState = {}

const tbl =  new $fgta5.Gridview('interfaceHeaderList-tbl')
const pnl_search = document.getElementById('interfaceHeaderList-pnl_search')
const btn_gridload = new $fgta5.ActionButton('interfaceHeaderList-btn_gridload') 

export const Section = CurrentSection
export const SearchParams = {}

export async function init(self, args) {
	console.log('initializing interfaceHeaderList ...')
	...
	try {
		...
		const obj_searchtext = document.getElementById('interfaceHeaderList-txt_searchtext')
		const btn_load = document.getElementById('interfaceHeaderList-btn_gridload')
		...
		// interfaceHeaderList-ext.mjs, export function initSearchParams(self, SearchParams) {} 
...
export function keyboardAction(self, actionName, evt) {
	if (actionName=='up') {
		...
		const interfaceHeaderEdit = self.Modules.interfaceHeaderEdit
		if (tbl.CurrentRow!=null) {
			interfaceHeaderEdit.Section.show({}, ()=>{
...
async function openRow(self, tr) {
	...
	const interfaceHeaderEdit = self.Modules.interfaceHeaderEdit
	interfaceHeaderEdit.clearForm(self, 'loading...')

	try {
		setCurrentRow(self, tr)
		CurrentState.SelectedRow.keyValue = keyvalue
		CurrentState.SelectedRow.key = key
		await interfaceHeaderEdit.openSelectedData(self, {key:key, keyvalue:keyvalue})
	} catch (err) {
		console.error(err)
	...
	setPagingButton(self, interfaceHeaderEdit)
}
...
function tbl_cellclick(self, evt) {
	...
	const interfaceHeaderEdit = self.Modules.interfaceHeaderEdit
	interfaceHeaderEdit.Section.show({}, (evt)=>{
		openRow(self, tr)
	})
...
async function tbl_loadData(self, params={}) {
	console.log('loading interfaceHeader data')
	console.log(params)
