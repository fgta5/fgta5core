...
const CurrentSectionId = Context.Sections.genderHeaderList
const CurrentSection = Crsl.Items[CurrentSectionId]
const CurrentState = {}

const tbl =  new $fgta5.Gridview('genderHeaderList-tbl')
const pnl_search = document.getElementById('genderHeaderList-pnl_search')
const btn_gridload = new $fgta5.ActionButton('genderHeaderList-btn_gridload') 

export const Section = CurrentSection
export const SearchParams = {}

export async function init(self, args) {
	console.log('initializing genderHeaderList ...')
	...
	try {
		...
		const obj_searchtext = document.getElementById('genderHeaderList-txt_searchtext')
		const btn_load = document.getElementById('genderHeaderList-btn_gridload')
		...
		// genderHeaderList-ext.mjs, export function initSearchParams(self, SearchParams) {} 
...
export function keyboardAction(self, actionName, evt) {
	if (actionName=='up') {
		...
		const genderHeaderEdit = self.Modules.genderHeaderEdit
		if (tbl.CurrentRow!=null) {
			genderHeaderEdit.Section.show({}, ()=>{
...
async function openRow(self, tr) {
	...
	const genderHeaderEdit = self.Modules.genderHeaderEdit
	genderHeaderEdit.clearForm(self, 'loading...')

	try {
		setCurrentRow(self, tr)
		CurrentState.SelectedRow.keyValue = keyvalue
		CurrentState.SelectedRow.key = key
		await genderHeaderEdit.openSelectedData(self, {key:key, keyvalue:keyvalue})

		// matikan atau nyalakan button prev/next sesuai kondisi
		setPagingButton(self, genderHeaderEdit)
...
function tbl_cellclick(self, evt) {
	...
	const genderHeaderEdit = self.Modules.genderHeaderEdit
	genderHeaderEdit.Section.show({}, (evt)=>{
		openRow(self, tr)
	})
...
async function tbl_loadData(self, params={}) {
	console.log('loading genderHeader data')
	console.log(params)
