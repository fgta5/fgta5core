import Context from './gender-context.mjs'
import * as Ext from './gender-ext.mjs'
...
const CurrentSectionId = Context.Sections.genderHeaderEdit
const CurrentSection = Crsl.Items[CurrentSectionId]
const Source = Context.Source


const TitleWhenNew = 'New Gender'
const TitleWhenView = 'View Gender'
const TitleWhenEdit = 'Edit Gender'
const EditModeText = 'Edit'
const LockModeText = 'Lock'

const btn_edit = new $fgta5.ActionButton('genderHeaderEdit-btn_edit')
const btn_save = new $fgta5.ActionButton('genderHeaderEdit-btn_save')
const btn_new = new $fgta5.ActionButton('genderHeaderEdit-btn_new', 'genderHeader-new')
const btn_del = new $fgta5.ActionButton('genderHeaderEdit-btn_delete')
const btn_reset = new $fgta5.ActionButton('genderHeaderEdit-btn_reset')
const btn_prev = new $fgta5.ActionButton('genderHeaderEdit-btn_prev')
const btn_next = new $fgta5.ActionButton('genderHeaderEdit-btn_next')


const btn_recordstatus = document.getElementById('genderHeader-btn_recordstatus')
const btn_logs = document.getElementById('genderHeader-btn_logs')
const btn_about = document.getElementById('genderHeader-btn_about')

const frm = new $fgta5.Form('genderHeaderEdit-frm');
const obj_gender_id = frm.Inputs['genderHeaderEdit-obj_gender_id']
const obj_gender_name = frm.Inputs['genderHeaderEdit-obj_gender_name']
const obj_gender_isnature = frm.Inputs['genderHeaderEdit-obj_gender_isnature']	
...
export async function init(self, args) {
	console.log('initializing genderHeaderEdit ...')
...
export async function openSelectedData(self, params) {
	...
	try {
		...
		const fn_iseditdisabled_name = 'genderHeaderEdit_isEditDisabled'
		...
		// export async function genderHeaderEdit_formOpened(self, frm, CurrentState)
		const fn_formopened_name = 'genderHeaderEdit_formOpened'
...
async function backToList(self, evt) {
	...
	if (goback) {
		frm.lock()
		const listId =  Context.Sections.genderHeaderList
...
async function  frm_locked(self, evt) {
	...
	// export function genderHeaderEdit_formLocked(self, frm, CurrentState) {}
	const fn_name = 'genderHeaderEdit_formLocked'
...
async function  frm_unlocked(self, evt) {
	...
	// export function genderHeaderEdit_formUnlocked(self, frm, CurrentState) {}
	const fn_name = 'genderHeaderEdit_formUnlocked'
	const fn = Extender[fn_name]
	if (typeof fn === 'function') {
		fn(self, frm, CurrentState)
	}
...
async function btn_new_click(self, evt) {
	...
	const genderHeaderList = self.Modules.genderHeaderList
	const listsecid = genderHeaderList.Section.Id
	...
	try {
		...
		const fn_newdata_name = 'genderHeaderEdit_newData'
		const fn_newdata = Extender[fn_newdata_name]
		if (typeof fn_newdata === 'function') {
			// export async function genderHeaderEdit_newData(self, datainit, frm) {}
		...
		if (fromListSection) {
			...
			self.Modules.genderHeaderList.Section.show()
		}
	}
}
...
async function btn_save_click(self, evt) {
	...
	// Extender Autofill
	const fn_autofill_name = 'genderHeaderEdit_autofill'
	...
	// Extender Saving
	// export async function genderHeaderEdit_dataSaving(self, dataToSave, frm, args) {}
	const args = { cancelSave: false }
	const fn_datasaving_name = 'genderHeaderEdit_dataSaving'
	...
	try {
		...
		// Extender Saving
		const fn_datasaved_name = 'genderHeaderEdit_dataSaved'
		const fn_datasaved = Extender[fn_datasaved_name]
		if (typeof fn_datasaved === 'function') {
			// export async function genderHeaderEdit_dataSaved(self, data, frm) {}
		...
		if (isNewData) {
			...
			self.Modules.genderHeaderList.addNewRow(self, data)
		} else {
			console.log('update data baris yang dibuka')
			self.Modules.genderHeaderList.updateCurrentRow(self, data)
		}
...
async function btn_del_click(self, evt) {
	...
	try {
		...
		self.Modules.genderHeaderList.removeCurrentRow(self)
...
async function btn_prev_click(self, evt) {
	...
	self.Modules.genderHeaderList.selectPreviousRow(self)
}

async function btn_next_click(self, evt) {
	console.log('btn_next_click')
	self.Modules.genderHeaderList.selectNextRow(self)
}
...
async function btn_recordstatus_click(self, evt) {
	...
	pageHelper.openSection(self, 'fRecord-section', params, async ()=>{
		...
		try {
			...
			const fn_addrecordinfo_name = 'genderHeaderEdit_addRecordInfo'
...
async function btn_about_click(self, evt) {
	...
	pageHelper.openSection(self, 'fAbout-section', params, async ()=>{
		...
		AboutSection.Title = 'About Gender'
		...
		if ( document.getElementById('fAbout-section-footer') == null) {
			...
			divFooter.innerHTML = 'This module is generated by fgta5 generator at 9 Dec 2025 13:59'
