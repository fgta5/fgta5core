...
const CurrentSectionId = Context.Sections.approvaltypeHeaderList
const CurrentSection = Crsl.Items[CurrentSectionId]
const CurrentState = {}

const tbl =  new $fgta5.Gridview('approvaltypeHeaderList-tbl')
const pnl_search = document.getElementById('approvaltypeHeaderList-pnl_search')
const btn_gridload = new $fgta5.ActionButton('approvaltypeHeaderList-btn_gridload') 

export const Section = CurrentSection
export const SearchParams = {}

export async function init(self, args) {
	console.log('initializing approvaltypeHeaderList ...')
	...
	try {
		...
		const obj_searchtext = document.getElementById('approvaltypeHeaderList-txt_searchtext')
		const btn_load = document.getElementById('approvaltypeHeaderList-btn_gridload')
		...
		// approvaltypeHeaderList-ext.mjs, export function initSearchParams(self, SearchParams) {} 
...
export function keyboardAction(self, actionName, evt) {
	if (actionName=='up') {
		...
		if (tbl.CurrentRow!=null) {
			approvaltypeHeaderEdit.Section.show({}, ()=>{
...
async function openRow(self, tr) {
	...
	const approvaltypeHeaderEdit = self.Modules.approvaltypeHeaderEdit
	approvaltypeHeaderEdit.clearForm(self, 'loading...')

	try {
		setCurrentRow(self, tr)
		CurrentState.SelectedRow.keyValue = keyvalue
		CurrentState.SelectedRow.key = key
		await approvaltypeHeaderEdit.openSelectedData(self, {key:key, keyvalue:keyvalue})

		// matikan atau nyalakan button prev/next sesuai kondisi
		setPagingButton(self, approvaltypeHeaderEdit)
...
function tbl_cellclick(self, evt) {
	...
	const approvaltypeHeaderEdit = self.Modules.approvaltypeHeaderEdit
	approvaltypeHeaderEdit.Section.show({}, (evt)=>{
		openRow(self, tr)
	})
...
async function tbl_loadData(self, params={}) {
	console.log('loading approvaltypeHeader data')
	console.log(params)
