...
const CurrentSectionId = Context.Sections.approvaltypeHeaderEdit
const CurrentSection = Crsl.Items[CurrentSectionId]
const Source = Context.Source


const TitleWhenNew = 'New Approval Type'
const TitleWhenView = 'View Approval Type'
const TitleWhenEdit = 'Edit Approval Type'
const EditModeText = 'Edit'
const LockModeText = 'Lock'

const btn_edit = new $fgta5.ActionButton('approvaltypeHeaderEdit-btn_edit')
const btn_save = new $fgta5.ActionButton('approvaltypeHeaderEdit-btn_save')
const btn_new = new $fgta5.ActionButton('approvaltypeHeaderEdit-btn_new', 'approvaltypeHeader-new')
const btn_del = new $fgta5.ActionButton('approvaltypeHeaderEdit-btn_delete')
const btn_reset = new $fgta5.ActionButton('approvaltypeHeaderEdit-btn_reset')
const btn_prev = new $fgta5.ActionButton('approvaltypeHeaderEdit-btn_prev')
const btn_next = new $fgta5.ActionButton('approvaltypeHeaderEdit-btn_next')


const btn_recordstatus = document.getElementById('approvaltypeHeader-btn_recordstatus')
const btn_logs = document.getElementById('approvaltypeHeader-btn_logs')
const btn_about = document.getElementById('approvaltypeHeader-btn_about')

const frm = new $fgta5.Form('approvaltypeHeaderEdit-frm');
const obj_approvaltype_id = frm.Inputs['approvaltypeHeaderEdit-obj_approvaltype_id']
const obj_approvaltype_name = frm.Inputs['approvaltypeHeaderEdit-obj_approvaltype_name']
const obj_approvaltype_descr = frm.Inputs['approvaltypeHeaderEdit-obj_approvaltype_descr']	
...
export async function init(self, args) {
	console.log('initializing approvaltypeHeaderEdit ...')
...
export async function openSelectedData(self, params) {
	...
	try {
		...
		const fn_iseditdisabled_name = 'approvaltypeHeaderEdit_isEditDisabled'
		...
		// export async function approvaltypeHeaderEdit_formOpened(self, frm, CurrentState)
		const fn_formopened_name = 'approvaltypeHeaderEdit_formOpened'
...
async function backToList(self, evt) {
	...
	if (goback) {
		frm.lock()
		const listId =  Context.Sections.approvaltypeHeaderList
...
async function  frm_locked(self, evt) {
	...
	// export function approvaltypeHeaderEdit_formLocked(self, frm, CurrentState) {}
	const fn_name = 'approvaltypeHeaderEdit_formLocked'
	...
	self.Modules.approvaltypeRoleList.headerLocked(self)
	self.Modules.approvaltypeRoleEdit.headerLocked(self)
...
async function  frm_unlocked(self, evt) {
	...
	// export function approvaltypeHeaderEdit_formUnlocked(self, frm, CurrentState) {}
	const fn_name = 'approvaltypeHeaderEdit_formUnlocked'
	const fn = Extender[fn_name]
	if (typeof fn === 'function') {
		fn(self, frm, CurrentState)
	}
	...
	self.Modules.approvaltypeRoleList.headerUnlocked(self)
	self.Modules.approvaltypeRoleEdit.headerUnlocked(self)	
...
async function btn_new_click(self, evt) {
	...
	const approvaltypeHeaderList = self.Modules.approvaltypeHeaderList
	const listsecid = approvaltypeHeaderList.Section.Id
	...
	try {
		...
		const fn_newdata_name = 'approvaltypeHeaderEdit_newData'
		const fn_newdata = Extender[fn_newdata_name]
		if (typeof fn_newdata === 'function') {
			// export async function approvaltypeHeaderEdit_newData(self, datainit, frm) {}
		...
		if (fromListSection) {
			...
			self.Modules.approvaltypeHeaderList.Section.show()
		}
	}
}
...
async function btn_save_click(self, evt) {
	...
	// Extender Autofill
	const fn_autofill_name = 'approvaltypeHeaderEdit_autofill'
	...
	// Extender Saving
	// export async function approvaltypeHeaderEdit_dataSaving(self, dataToSave, frm, args) {}
	const args = { cancelSave: false }
	const fn_datasaving_name = 'approvaltypeHeaderEdit_dataSaving'
	...
	try {
		...
		// Extender Saving
		const fn_datasaved_name = 'approvaltypeHeaderEdit_dataSaved'
		const fn_datasaved = Extender[fn_datasaved_name]
		if (typeof fn_datasaved === 'function') {
			// export async function approvaltypeHeaderEdit_dataSaved(self, data, frm) {}
		...
		if (isNewData) {
			...
			self.Modules.approvaltypeHeaderList.addNewRow(self, data)
		} else {
			console.log('update data baris yang dibuka')
			self.Modules.approvaltypeHeaderList.updateCurrentRow(self, data)
		}
...
async function btn_del_click(self, evt) {
	...
	try {
		...
		self.Modules.approvaltypeHeaderList.removeCurrentRow(self)
...
async function btn_prev_click(self, evt) {
	...
	self.Modules.approvaltypeHeaderList.selectPreviousRow(self)
}

async function btn_next_click(self, evt) {
	console.log('btn_next_click')
	self.Modules.approvaltypeHeaderList.selectNextRow(self)
}
...
async function btn_recordstatus_click(self, evt) {
	...
	pageHelper.openSection(self, 'fRecord-section', params, async ()=>{
		...
		try {
			...
			const fn_addrecordinfo_name = 'approvaltypeHeaderEdit_addRecordInfo'
...
async function btn_about_click(self, evt) {
	...
	pageHelper.openSection(self, 'fAbout-section', params, async ()=>{
		...
		AboutSection.Title = 'About Approval Type'
		...
		if ( document.getElementById('fAbout-section-footer') == null) {
			...
			divFooter.innerHTML = 'This module is generated by fgta5 generator at 9 Dec 2025 10:41'
