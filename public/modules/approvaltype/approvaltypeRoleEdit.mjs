...
import * as Ext from './approvaltype-ext.mjs'
...
const CurrentSectionId = Context.Sections.approvaltypeRoleEdit
const CurrentSection = Crsl.Items[CurrentSectionId]
const Source = Context.Source

const TitleWhenNew = 'New Role'
const TitleWhenView = 'View Role'
const TitleWhenEdit = 'Edit Role'
const EditModeText = 'Edit'
const LockModeText = 'Lock'



const btn_edit = new $fgta5.ActionButton('approvaltypeRoleEdit-btn_edit')
const btn_save = new $fgta5.ActionButton('approvaltypeRoleEdit-btn_save')
const btn_new = new $fgta5.ActionButton('approvaltypeRoleEdit-btn_new', 'approvaltypeRole-addrow')
const btn_del = new $fgta5.ActionButton('approvaltypeRoleEdit-btn_delete', 'approvaltypeRole-delrow')
const btn_reset = new $fgta5.ActionButton('approvaltypeRoleEdit-btn_reset')
const btn_prev = new $fgta5.ActionButton('approvaltypeRoleEdit-btn_prev')
const btn_next = new $fgta5.ActionButton('approvaltypeRoleEdit-btn_next')

const btn_recordstatus = document.getElementById('approvaltypeRole-btn_recordstatus')
const btn_logs = document.getElementById('approvaltypeRole-btn_logs')

const frm = new $fgta5.Form('approvaltypeRoleEdit-frm');
const obj_approvaltyperole_id = frm.Inputs['approvaltypeRoleEdit-obj_approvaltyperole_id']
const obj_approvalsigntype_id = frm.Inputs['approvaltypeRoleEdit-obj_approvalsigntype_id']
const obj_approval_signtitle = frm.Inputs['approvaltypeRoleEdit-obj_approval_signtitle']
const obj_isfinalapproval = frm.Inputs['approvaltypeRoleEdit-obj_isfinalapproval']
const obj_approvaltyperole_descr = frm.Inputs['approvaltypeRoleEdit-obj_approvaltyperole_descr']
const obj_rolelevel_id = frm.Inputs['approvaltypeRoleEdit-obj_rolelevel_id']
const obj_role_id = frm.Inputs['approvaltypeRoleEdit-obj_role_id']
const obj_approvaltyperole_order = frm.Inputs['approvaltypeRoleEdit-obj_approvaltyperole_order']
const obj_approvaltype_id = frm.Inputs['approvaltypeRoleEdit-obj_approvaltype_id']	
...
export async function init(self, args) {
	...
	CurrentState.getHeaderForm = () => {
		const approvaltypeHeaderEdit = self.Modules.approvaltypeHeaderEdit
		const frmHeader = approvaltypeHeaderEdit.getHeaderForm()
		return frmHeader
	}


	
	// Combobox: obj_approvalsigntype_id
	obj_approvalsigntype_id.addEventListener('selecting', async (evt)=>{
		const fn_selecting_name = 'obj_approvalsigntype_id_selecting'
		const fn_selecting = Extender[fn_selecting_name]
		if (typeof fn_selecting === 'function') {
			// create function di Extender (jika perlu):
			// export async function obj_approvalsigntype_id_selecting(self, obj_approvalsigntype_id, frm, evt) {}
			fn_selecting(self, obj_approvalsigntype_id, frm, evt)
		} else {
			// default selecting
			const cbo = evt.detail.sender
			const dialog = evt.detail.dialog
			const searchtext = evt.detail.searchtext!=null ? evt.detail.searchtext : ''
			const url = 'approvalsigntype/header-list'
			const sort = {}
			const criteria = {
			...
			// export function obj_approvalsigntype_id_selecting_criteria(self, obj_approvalsigntype_id, frm, criteria, sort, evt) {}
			const fn_selecting_criteria_name = 'obj_approvalsigntype_id_selecting_criteria'
			const fn_selecting_criteria = Extender[fn_selecting_criteria_name]
			if (typeof fn_selecting_criteria === 'function') {
				fn_selecting_criteria(self, obj_approvalsigntype_id, frm, criteria, sort, evt)
			}
			...
			try {
				...
				for (var row of result.data) {
					evt.detail.addRow(row.approvalsigntype_id, row.approvalsigntype_name, row)
				}
...
export async function openSelectedData(self, params) {
	...
	try {
		...
		if (!CurrentState.headerFormLocked) {
			const fn_iseditdisabled_name = 'approvaltypeRoleEdit_isEditDisabled'
		...
		// export function approvaltypeRoleEdit_formOpened(self, frm, CurrentState) {}
		const fn_formopened_name = 'approvaltypeRoleEdit_formOpened'
...
export function headerLocked(self) {
	...
	// export function approvaltypeRoleEdit_formLocked(self, frm, CurrentState) {}
	const fn_name = 'approvaltypeRoleEdit_formLocked'
...
export function headerUnlocked(self) {
	...
	// export function approvaltypeRoleEdit_formUnlocked(self, frm, CurrentState) {}
	const fn_name = 'approvaltypeRoleEdit_formUnlocked'
	const fn = Extender[fn_name]
	if (typeof fn === 'function') {
		fn(self, frm, CurrentState)
	}	
...
async function backToList(self, evt) {
	...
	if (goback) {
		frm.lock()
		const listId =  Context.Sections.approvaltypeRoleList
...
async function  frm_unlocked(self, evt) {
	...
	// Extender untuk event Unlocked
	// export function approvaltypeRoleEdit_formUnlocked(self, frm) {}
	const fn_name = 'approvaltypeRoleEdit_formUnlocked'
	const fn = Extender[fn_name]
	if (typeof fn === 'function') {
		fn(self, frm)
...
async function btn_new_click(self, evt) {
	...
	const approvaltypeRoleList = self.Modules.approvaltypeRoleList
	const listsecid = approvaltypeRoleList.Section.Id
	const fromListSection = sourceSection===listsecid

	if (fromListSection) {
		console.log('tambahkan row baru')
		CurrentSection.setSectionReturn(approvaltypeRoleList.Section)
	...
	try {
		...
		const approvaltype_id = header_pk.value

		// inisiasi data baru
		const datainit = {
			approvaltype_id,
			approvaltyperole_order: 0,
		}
		...
		// export async function approvaltypeRoleEdit_newData(self, datainit, frm, CurrentState) {}
		const fn_newdata_name = 'approvaltypeRoleEdit_newData'
		...
		if (fromListSection) {
			...
			self.Modules.approvaltypeRoleList.Section.show()
		}
	}
}
...
async function btn_save_click(self, evt) {
	...
	// Extender Autofill
	// export async function approvaltypeRoleEdit_autofill(self, frm) {}
	const fn_autofill_name = 'approvaltypeRoleEdit_autofill'
	...
	// Extender Saving
	// export async function approvaltypeRoleEdit_dataSaving(self, dataToSave, frm, args) {}
	const args = { cancelSave: false }
	const fn_datasaving_name = 'approvaltypeRoleEdit_dataSaving'
	...
	try {
		...
		// Extender Saving
		// export async function approvaltypeRoleEdit_dataSaved(self, data, frm) {}
		const fn_datasaved_name = 'approvaltypeRoleEdit_dataSaved'
		...
		if (isNewData) {
			...
			self.Modules.approvaltypeRoleList.addNewRow(self, data)
		} else {
			console.log('update data baris yang dibuka')
			self.Modules.approvaltypeRoleList.updateCurrentRow(self, data)
		}
...
async function btn_del_click(self, evt) {
	...
	try {
		...
		self.Modules.approvaltypeRoleList.removeCurrentRow(self)
...
async function btn_prev_click(self, evt) {
	...
	self.Modules.approvaltypeRoleList.selectPreviousRow(self)
}

async function btn_next_click(self, evt) {
	console.log('btn_next_click')
	self.Modules.approvaltypeRoleList.selectNextRow(self)
}
...
async function btn_recordstatus_click(self, evt) {
	...
	pageHelper.openSection(self, 'fRecord-section', params, async ()=>{
		...
		try {
			...
			// melalui extender approvaltypeRoleEdit_addRecordInfo
			// export async function approvaltypeRoleEdit_addRecordInfo(self,  data) {}
			const fn_addrecordinfo_name = 'approvaltypeRoleEdit_addRecordInfo'
