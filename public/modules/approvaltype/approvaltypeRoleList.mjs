...
const CurrentSectionId = Context.Sections.approvaltypeRoleList
const CurrentSection = Crsl.Items[CurrentSectionId]
const CurrentState = {}

const tbl =  new $fgta5.Gridview('approvaltypeRoleList-tbl')

const btn_addrow = document.getElementById('approvaltypeRoleList-btn_addrow') // tidak perlu pakai action, karna action didefine di edit
const btn_delrow = new $fgta5.ActionButton('approvaltypeRoleList-btn_delrow')
...
export async function init(self, args) {
	...
	CurrentSection.addEventListener($fgta5.Section.EVT_BACKBUTTONCLICK, async (evt)=>{
		const sectionId =  Context.Sections.approvaltypeHeaderEdit
	...
	// export function roleList_addTableEvents(self, tbl) {}
	const fn_addTableEvents_name = 'roleList_addTableEvents'
	...
	// Extend list detil
	// export function approvaltypeRoleList_init(self) {}
	const fn_name = 'approvaltypeRoleList_init'
	const fn_approvaltypeRoleList_init = Extender[fn_name]
	if (typeof fn_approvaltypeRoleList_init === 'function') {
		fn_approvaltypeRoleList_init(self)
	}
...
export async function openList(self, params) {
	...
	// bisa di set pada Extender.approvaltypeRoleList_openList :  bisa menggunakan template untuk di embed ke header pada detil list
	// export function approvaltypeRoleList_openList(self, headerForm) {}
	const fn_name = 'approvaltypeRoleList_openList'
	const fn_approvaltypeRoleList_openList = Extender[fn_name]
	if (typeof fn_approvaltypeRoleList_openList === 'function') {
		fn_approvaltypeRoleList_openList(self, headerForm)
	}
...
export function keyboardAction(self, actionName) {
	if (actionName=='up') {
		...
		const approvaltypeRoleEdit = self.Modules.approvaltypeRoleEdit
		if (tbl.CurrentRow!=null) {
			approvaltypeRoleEdit.Section.show({}, (evt)=>{
...
async function openRow(self, tr) {
	...
	const approvaltypeRoleEdit = self.Modules.approvaltypeRoleEdit
	approvaltypeRoleEdit.clearForm(self, 'loading...')

	try {
		setCurrentRow(self, tr)
		CurrentState.SelectedRow.keyValue = keyvalue
		CurrentState.SelectedRow.key = key
		await approvaltypeRoleEdit.openSelectedData(self, {key:key, keyvalue:keyvalue})
	} catch (err) {
		console.error(err)
	...
	setPagingButton(self, approvaltypeRoleEdit)
}
...
async function listRows(self, criteria, offset, limit, sort) {
	...
	const fn_dataLoad_name = 'roleList_dataLoad'
