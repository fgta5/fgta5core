import Context from './approval-context.mjs'
import * as Ext from './approval-ext.mjs'
...
const CurrentSectionId = Context.Sections.approvalHeaderEdit
const CurrentSection = Crsl.Items[CurrentSectionId]
const Source = Context.Source


const TitleWhenNew = 'New Approval'
const TitleWhenView = 'View Approval'
const TitleWhenEdit = 'Edit Approval'
const EditModeText = 'Edit'
const LockModeText = 'Lock'

const btn_edit = new $fgta5.ActionButton('approvalHeaderEdit-btn_edit')
const btn_save = new $fgta5.ActionButton('approvalHeaderEdit-btn_save')
const btn_new = new $fgta5.ActionButton('approvalHeaderEdit-btn_new', 'approvalHeader-new')
const btn_del = new $fgta5.ActionButton('approvalHeaderEdit-btn_delete')
const btn_reset = new $fgta5.ActionButton('approvalHeaderEdit-btn_reset')
const btn_prev = new $fgta5.ActionButton('approvalHeaderEdit-btn_prev')
const btn_next = new $fgta5.ActionButton('approvalHeaderEdit-btn_next')


const btn_recordstatus = document.getElementById('approvalHeader-btn_recordstatus')
const btn_logs = document.getElementById('approvalHeader-btn_logs')
const btn_about = document.getElementById('approvalHeader-btn_about')

const frm = new $fgta5.Form('approvalHeaderEdit-frm');
const obj_approval_id = frm.Inputs['approvalHeaderEdit-obj_approval_id']
const obj_approval_doc = frm.Inputs['approvalHeaderEdit-obj_approval_doc']
const obj_approvaltype_id = frm.Inputs['approvalHeaderEdit-obj_approvaltype_id']	
...
export async function init(self, args) {
	console.log('initializing approvalHeaderEdit ...')
...
export async function openSelectedData(self, params) {
	...
	try {
		...
		const fn_iseditdisabled_name = 'approvalHeaderEdit_isEditDisabled'
		...
		// export async function approvalHeaderEdit_formOpened(self, frm, CurrentState)
		const fn_formopened_name = 'approvalHeaderEdit_formOpened'
...
async function backToList(self, evt) {
	...
	if (goback) {
		frm.lock()
		const listId =  Context.Sections.approvalHeaderList
...
async function  frm_locked(self, evt) {
	...
	// export function approvalHeaderEdit_formLocked(self, frm, CurrentState) {}
	const fn_name = 'approvalHeaderEdit_formLocked'
...
async function  frm_unlocked(self, evt) {
	...
	// export function approvalHeaderEdit_formUnlocked(self, frm, CurrentState) {}
	const fn_name = 'approvalHeaderEdit_formUnlocked'
	const fn = Extender[fn_name]
	if (typeof fn === 'function') {
		fn(self, frm, CurrentState)
	}
...
async function btn_new_click(self, evt) {
	...
	const approvalHeaderList = self.Modules.approvalHeaderList
	const listsecid = approvalHeaderList.Section.Id
	...
	try {
		...
		const fn_newdata_name = 'approvalHeaderEdit_newData'
		const fn_newdata = Extender[fn_newdata_name]
		if (typeof fn_newdata === 'function') {
			// export async function approvalHeaderEdit_newData(self, datainit, frm) {}
		...
		if (fromListSection) {
			...
			self.Modules.approvalHeaderList.Section.show()
		}
	}
}
...
async function btn_save_click(self, evt) {
	...
	// Extender Autofill
	const fn_autofill_name = 'approvalHeaderEdit_autofill'
	...
	// Extender Saving
	// export async function approvalHeaderEdit_dataSaving(self, dataToSave, frm, args) {}
	const args = { cancelSave: false }
	const fn_datasaving_name = 'approvalHeaderEdit_dataSaving'
	...
	try {
		...
		// Extender Saving
		const fn_datasaved_name = 'approvalHeaderEdit_dataSaved'
		const fn_datasaved = Extender[fn_datasaved_name]
		if (typeof fn_datasaved === 'function') {
			// export async function approvalHeaderEdit_dataSaved(self, data, frm) {}
		...
		if (isNewData) {
			...
			self.Modules.approvalHeaderList.addNewRow(self, data)
		} else {
			console.log('update data baris yang dibuka')
			self.Modules.approvalHeaderList.updateCurrentRow(self, data)
		}
...
async function btn_del_click(self, evt) {
	...
	try {
		...
		self.Modules.approvalHeaderList.removeCurrentRow(self)
...
async function btn_prev_click(self, evt) {
	...
	self.Modules.approvalHeaderList.selectPreviousRow(self)
}

async function btn_next_click(self, evt) {
	console.log('btn_next_click')
	self.Modules.approvalHeaderList.selectNextRow(self)
}
...
async function btn_recordstatus_click(self, evt) {
	...
	pageHelper.openSection(self, 'fRecord-section', params, async ()=>{
		...
		try {
			...
			const fn_addrecordinfo_name = 'approvalHeaderEdit_addRecordInfo'
...
async function btn_about_click(self, evt) {
	...
	pageHelper.openSection(self, 'fAbout-section', params, async ()=>{
		...
		AboutSection.Title = 'About Approval'
		...
		if ( document.getElementById('fAbout-section-footer') == null) {
			...
			divFooter.innerHTML = 'This module is generated by fgta5 generator at 8 Dec 2025 09:59'
