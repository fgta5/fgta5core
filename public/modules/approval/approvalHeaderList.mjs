...
const CurrentSectionId = Context.Sections.approvalHeaderList
const CurrentSection = Crsl.Items[CurrentSectionId]
const CurrentState = {}

const tbl =  new $fgta5.Gridview('approvalHeaderList-tbl')
const pnl_search = document.getElementById('approvalHeaderList-pnl_search')
const btn_gridload = new $fgta5.ActionButton('approvalHeaderList-btn_gridload') 

export const Section = CurrentSection
export const SearchParams = {}

export async function init(self, args) {
	console.log('initializing approvalHeaderList ...')
	...
	try {
		...
		const obj_searchtext = document.getElementById('approvalHeaderList-txt_searchtext')
		const btn_load = document.getElementById('approvalHeaderList-btn_gridload')
		...
		// approvalHeaderList-ext.mjs, export function initSearchParams(self, SearchParams) {} 
...
export function keyboardAction(self, actionName, evt) {
	if (actionName=='up') {
		...
		const approvalHeaderEdit = self.Modules.approvalHeaderEdit
		if (tbl.CurrentRow!=null) {
			approvalHeaderEdit.Section.show({}, ()=>{
...
async function openRow(self, tr) {
	...
	const approvalHeaderEdit = self.Modules.approvalHeaderEdit
	approvalHeaderEdit.clearForm(self, 'loading...')

	try {
		setCurrentRow(self, tr)
		CurrentState.SelectedRow.keyValue = keyvalue
		CurrentState.SelectedRow.key = key
		await approvalHeaderEdit.openSelectedData(self, {key:key, keyvalue:keyvalue})

		// matikan atau nyalakan button prev/next sesuai kondisi
		setPagingButton(self, approvalHeaderEdit)
...
function tbl_cellclick(self, evt) {
	...
	const approvalHeaderEdit = self.Modules.approvalHeaderEdit
	approvalHeaderEdit.Section.show({}, (evt)=>{
		openRow(self, tr)
	})
...
async function tbl_loadData(self, params={}) {
	console.log('loading approvalHeader data')
	console.log(params)
