...
const CurrentSectionId = Context.Sections.userLoginList
const CurrentSection = Crsl.Items[CurrentSectionId]
const CurrentState = {}

const tbl =  new $fgta5.Gridview('userLoginList-tbl')

const btn_addrow = document.getElementById('userLoginList-btn_addrow') // tidak perlu pakai action, karna action didefine di edit
const btn_delrow = new $fgta5.ActionButton('userLoginList-btn_delrow')
...
export async function init(self, args) {
	...
	// export function loginList_addTableEvents(self, tbl) {}
	const fn_addTableEvents_name = 'loginList_addTableEvents'
	...
	// Extend list detil
	// export function userLoginList_init(self) {}
	const fn_name = 'userLoginList_init'
	const fn_userLoginList_init = Extender[fn_name]
	if (typeof fn_userLoginList_init === 'function') {
		fn_userLoginList_init(self)
	}
...
export async function openList(self, params) {
	...
	// bisa di set pada Extender.userLoginList_openList :  bisa menggunakan template untuk di embed ke header pada detil list
	// export function userLoginList_openList(self, headerForm) {}
	const fn_name = 'userLoginList_openList'
	const fn_userLoginList_openList = Extender[fn_name]
	if (typeof fn_userLoginList_openList === 'function') {
		fn_userLoginList_openList(self, headerForm)
	}
...
export function keyboardAction(self, actionName) {
	if (actionName=='up') {
		...
		if (tbl.CurrentRow!=null) {
			userLoginEdit.Section.show({}, (evt)=>{
...
async function openRow(self, tr) {
	...
	const userLoginEdit = self.Modules.userLoginEdit
	userLoginEdit.clearForm(self, 'loading...')

	try {
		setCurrentRow(self, tr)
		CurrentState.SelectedRow.keyValue = keyvalue
		CurrentState.SelectedRow.key = key
		await userLoginEdit.openSelectedData(self, {key:key, keyvalue:keyvalue})
	} catch (err) {
		console.error(err)
	...
	setPagingButton(self, userLoginEdit)
}


async function listRows(self, criteria, offset, limit, sort) {
	const url = `/${Context.moduleName}/login-list`
	const evt = { url, limit }

	// export function loginList_dataLoad(self, criteria, sort, evt) {}
	const fn_dataLoad_name = 'loginList_dataLoad'
...
async function deleteRows(self, data) {
	const url = `/${Context.moduleName}/login-delete-rows`
	try {
...
async function tbl_loadData(self, params={}) {
	...
	try {
		...
		// export function loginList_tableDataLoaded(self, tbl, result) {}
		const fn_name = 'loginList_tableDataLoaded'
